CREATE TABLE waka_daily_summary (
                              ...
                              PRIMARY KEY (waka_id, summary_date)
);

COMMENT ON TABLE waka_daily_summary IS '사용자별 일자별 코딩 요약';

COMMENT ON COLUMN waka_daily_summary.waka_id IS '사용자 ID';
COMMENT ON COLUMN waka_daily_summary.summary_date IS '코딩 요약 일자 (KST 기준)';
COMMENT ON COLUMN waka_daily_summary.start_time IS '해당 일자의 시작 시각 (UTC)';
COMMENT ON COLUMN waka_daily_summary.end_time IS '해당 일자의 종료 시각 (UTC)';
COMMENT ON COLUMN waka_daily_summary.timezone IS '사용자 타임존';
COMMENT ON COLUMN waka_daily_summary.total_seconds IS '총 코딩 시간 (초 단위)';
COMMENT ON COLUMN waka_daily_summary.digital_time IS '디지털 시계 형식 (예: 1:30:05)';
COMMENT ON COLUMN waka_daily_summary.text_time IS '사람이 읽기 쉬운 시간 표현 (예: 1 hr 30 mins)';
COMMENT ON COLUMN waka_daily_summary.created_at IS '레코드 생성 시각';
...
COMMENT ON COLUMN waka_daily_summary_project.waka_id IS '사용자 ID';
...
COMMENT ON COLUMN waka_summary_language.waka_id IS '사용자 ID';
...
COMMENT ON COLUMN waka_summary_editor.waka_id IS '사용자 ID';
...
COMMENT ON COLUMN waka_summary_os.waka_id IS '사용자 ID';
...
COMMENT ON COLUMN waka_summary_dependency.waka_id IS '사용자 ID';
...
COMMENT ON COLUMN waka_summary_machine.waka_id IS '사용자 ID';
